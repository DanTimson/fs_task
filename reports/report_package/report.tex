...
The task for this project is to compare optimization strategies for Qwen2.5-0.5B LoRA fine-tuning using training time, maximum memory usage, final model quality, and convergence as the main criteria.
...
The training code uses Hugging Face Transformers and PEFT, but the training loop itself is implemented manually.
...
All main experiments used the same workload:
    5,000 samples from \texttt{Elriggs/openwebtext-100k}, sequence length 512, batch size 4 and 2 epochs.
...
For each run, the following quantities were recorded:
    total training time, peak GPU memory, average loss over the last 10 logged points, final logged loss, average per-step time and average optimizer-step time.
...
PIQA results are shown in Table~\ref{tab:piqa}.
...
Model & Accuracy \\
\midrule
Base Qwen2.5-0.5B & 0.7051 \\
AdamW fine-tuned & 0.6997 \\
Muon fine-tuned & 0.6997 \\
\bottomrule
\end{tabular}
\end{table}

\section{Discussion}
The PIQA results show that the fine-tuning corpus and evaluation benchmark matter.
